...
| Claim ID | Claim | Previous state | Evidence | New state | Scope |
|---|---|---|---|---|---|
| SCL-010 | Shopee BR offers an Open Platform application/onboarding process for sellers/integrators | `SEARCH_INDEXED` (article titles 3445, 27314 — bodies unread) | SPD-002,SPD-003,SPD-004,SPD-008,SPD-009 | PRIMARY_VERIFIED — documented BR developer journey (login→dev profile→app→sandbox→Go Live→authorize); Console at `open.shopee.com/console` | `BRAZIL` |
| SCL-011 | Which seller/account types are eligible for BR Product API access | `UNRESOLVED` | SPD-004 | PRIMARY_VERIFIED — BR: only `Registered Business Seller` and `Third-party Partner (ISV)` developer types | `BRAZIL` |
| SCL-012 | Whether BR Product API access requires approved-partner status / manual approval | `UNRESOLVED` (evidence leans approval-gated; **not** `CONFIRMED_RESTRICTED`) | SPD-003,SPD-004,SPD-005,SPD-009 | PRIMARY_VERIFIED — developer PROFILE is approval-gated (Shopee internal review); PRODUCTION is Go-Live-review-gated; listing-API app categories (`Seller In-House System` / `ERP System`) are NOT whitelist-gated (whitelist only for Swarm ERP / Brand Membership / Auto Parts SPI apps, SPD-035) | `BRAZIL` |
...
| SCL-033 | **Whether a no-variation Item has a hidden/default Model** | `UNRESOLVED` | SPD-011,SPD-016,SPD-027 | PRIMARY_VERIFIED — Shopee keeps an internal **default model** for no-variation items; addressed as `model_id = 0`; `get_item_base_info.is_fulfillment_by_shopee` references 'only has a default model' | `GLOBAL_API` |
...
| SCL-122 | Title max/min length | `SEARCH_INDEXED` (≈255/256 — provisional, unlocked) | SPD-029,SPD-010 | CORRECTED — title limit is `get_item_limit.item_name_length_limit` (sample min 5 / max 100; DYNAMIC per shop+category). NOT ≈255. Errors `error_title_exceeds_max_length` / `error_item_name_is_too_short` / `error_title_character_forbidden` | `UNKNOWN_SCOPE` |
| SCL-123 | Description max length; `extended` description availability | `SEARCH_INDEXED` (≈5,000 — provisional, unlocked) | SPD-029 | CORRECTED — description limit is `get_item_limit.item_description_length_limit` (sample min 10 / max 2000; DYNAMIC). `extended_description_limit` separate object (text len, image num/width/height/aspect). Hashtags ≤18. Extended description = whitelist sellers | `UNKNOWN_SCOPE` |
...
| SCL-141 | No-variation mapping: `variant_id → item_id`, OR `variant_id → hidden model_id` if one exists | `UNRESOLVED` (depends on SCL-033) | SPD-016, SPD-027, SPD-011 | CORRECTED / PRIMARY_VERIFIED — `variant_id → (item_id, model_id = 0)`; Shopee's internal default model is addressed as `model_id 0`; no separate hidden id is exposed | — |
